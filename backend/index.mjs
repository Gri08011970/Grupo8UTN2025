...
const FRONT_ORIGIN = process.env.FRONT_ORIGIN || "";    // prod (Static host)
...
// ======================================================
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        return res.status(201).json({ user: { id, name, email: emailL, phone: phone || "", role }, token });
      } catch (e) {
        console.error(e);
  ...
  // ----- Validaciones productos/órdenes -----
  ...
  const validateOrderStatusPatch = [ body("status").isIn(STATUS).withMessage("Estado inválido"), validateOr400 ];
  ...
  // Validaciones ANTES del router json-server
  ...
  // Completar campos al crear orden
